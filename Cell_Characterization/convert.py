...
if __name__ == "__main__":
    ...
    print(f"Converting PVs of file type {file_type_to_convert} in path {path} to type {file_type_to_convert_to}")

    file_paths = sorted([file_path for file_path in dir_list if os.path.splitext(file_path)[1] == file_type_to_convert])
    for file_path in file_paths:
        with open(f"{path}{file_path}", mode="r", encoding='utf-8') as file:
            print(f"Converting {path}{file_path}...")
            convert(f"{path}{os.path.splitext(file_path)[0]}", file, file_type_to_convert_to)
